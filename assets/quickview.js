 ...
 * products (e.g. a fixed-size pendant) get an inline add-to-cart.
 */
(function () {
  'use strict';
  ...
  function render(product) {
    ...
    var simple = (product.options || []).length <= 1 && (product.options[0] ? product.options[0].values.length <= 6 : true);

    var optionsHtml = '';
    var ctaHtml = '';

    if (simple && product.options.length === 1) {
    ...
    var priceHtml = product.variants.every(function (v) { return v.price === variant.price; })
    ...
    if (addBtn) {
      addBtn.addEventListener('click', function () {
        ...
        fetch('/cart/add.js', {
          ...
          body: JSON.stringify({ id: addBtn.getAttribute('data-variant-id'), quantity: 1 })
        })
